...
function getStepStates(status: ProcessingStatus, validationStatus: ValidationStatus | null): Array<{ label: string; state: StepState }> {
  ...
  const progressUpdated = status === "completed";

  return [
    { label: "Photos reçues", state: "done" },
    {
    ...
    {
      label: "Mise à jour de la progression",
      state:
        validationStatus === "MISMATCH" || validationStatus === "UNREADABLE"
          ? "blocked"
          : progressUpdated
...
function StepIndicator({ state }: { state: StepState }) {
  ...
  if (state === "current") {
    return <span aria-hidden="true" className="inline-block h-4 w-4 animate-spin rounded-full border-2 border-current border-r-transparent align-[-0.125rem]" />;
  }
  return <span aria-hidden="true">•</span>;
}

export function PracticeAnalysisStatus({
  submissionId,
  initialStatus,
  ...
  const pollStatus = useEffectEvent(async () => {
    const response = await fetch(`/api/app/submissions/${submissionId}/status`, {
      cache: "no-store",
    });
    const payload = await response.json() as PracticeStatusPayload;
    if (!response.ok || !payload.ok) {
      setError(payload.error ?? "Impossible de récupérer l'état de la soumission.");
      return;
    }
  ...
  async function handleRetry() {
    ...
    setError("");
    try {
      const response = await fetch(`/api/app/submissions/${submissionId}/retry`, {
        method: "POST",
        headers: { "x-elan-ajax": "1" },
      });
      const payload = await response.json() as PracticeStatusPayload;
      if (!response.ok || !payload.ok) {
  ...
  return (
    <div className="space-y-6">
      <section className="card">
        <p className="text-sm font-semibold uppercase tracking-wide text-blue-700">Travail envoyé</p>
        ...
        <div className="mt-4 space-y-3 text-sm leading-6 text-slate-700">
          {status === "pending" && validationStatus === null ? <p>Les fichiers ont bien été reçus. ÉLAN vérifie d’abord qu’il s’agit bien de la copie attendue avant toute analyse pédagogique.</p> : null}
          {status === "processing" ? <p>ÉLAN vérifie la cohérence de la copie, analyse les réponses puis met à jour la progression de votre enfant.</p> : null}
          {status === "failed" && validationStatus === "PARTIAL_MATCH" ? <p>La copie semble correspondre en partie au travail attendu. Si c’est bien la bonne copie, vous pouvez confirmer pour lancer l’analyse.</p> : null}
          {status === "failed" && validationStatus === "MISMATCH" ? <p>Cette copie ne semble pas correspondre au travail demandé. Renvoyez la bonne copie de la séance.</p> : null}
        ...
        {canConfirm ? (
          <div className="mt-5 flex flex-wrap gap-3">
            ...
            <a href="/app/envoyer-travail" className="btn-secondary">Envoyer une autre photo</a>
          </div>
        ) : null}
        {canRetry ? (
          <div className="mt-5 flex flex-wrap gap-3">
